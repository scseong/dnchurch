...
- GitHub Pull Request 리뷰 요약과 인라인 리뷰 코멘트는 모두 한국어로 작성한다.
- 코드 식별자, 파일 경로, API 이름, 명령어, 인용된 오류 메시지는 원문 언어를 유지한다.
- 리뷰는 발견 사항부터 시작한다. 정확성, 보안, 데이터 손실, 인증/캐시/배포 회귀, 누락된 테스트를 우선순위로 본다.
- 간결한 한국어로 작성한다. 기술 식별자는 번역하지 않는다.
