...
**산출 문서의 가독성을 기계로 검사한다.** `scripts/check-readability.mjs`가 코드 span 밖 산문에서 문장당 가운뎃점 수와 표 셀 길이·구분자 수를 잰다. 같은 표현 문제(가운뎃점 나열, 키워드 명사 더미)가 모든 문서에서 반복됐는데, writing-style SKILL의 규칙은 있었으나 생성 경로에 붙지 않아 안 지켜졌다. 규칙을 더 얹는 대신 기계 검사를 둔다. 병목을 피하려고 처음엔 전부 경고만 하고, 오탐이 적으면 나중에 PR 본문 한 지점만 차단으로 올린다.
