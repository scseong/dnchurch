...
- **컨테이너 2종 혼재** (V1-1): `LayoutContainer` 10곳·`MainContainer` 6곳. 선택 기준이 없어 같은 about 도메인 안에서도 갈린다. 실측 차이는 padding이다 — 상하(`LayoutContainer` 0·`MainContainer` `$spacing-64`)와 좌우(`LayoutContainer` `$container-padding`·`MainContainer` `$spacing-20`)가 모두 다르다. `MainContainer`의 `title` prop은 렌더가 주석 처리돼 화면에 안 나오는 dead prop이다.
...
1. **컨테이너 단일화** — `LayoutContainer`·`MainContainer`를 `PageContainer` 1종으로 합친다. 상하 여백은 variant prop으로 고른다(`flush` = 0, `padded` = `$spacing-64`). 좌우 padding은 `$container-padding`으로 통일한다 — `MainContainer`의 `$spacing-20`을 쓰던 news 계열은 본문 폭이 바뀌므로 Phase 4 마이그레이션에서 시각 확인한다. `MainContainer`의 dead `title` prop과 미사용 `.title` 스타일은 제거한다.
...
- Phase 4 마이그레이션 비용이 생긴다 — 컨테이너를 쓰는 16곳(`LayoutContainer` 10·`MainContainer` 6)의 import 교체, `welcome`·`vision` full-width 전환, news 랜딩 신규 구현.
- 자체 hero 페이지를 새로 만들 때 `SELF_HERO_PATHS` 등록을 잊으면 자동 Hero가 또 겹친다. 등록 규약을 `page-patterns.md`에 적어 둔다.

### 영향 범위

- 코드: `src/components/layout/container/`(PageContainer 신설), `Hero/hero.config.ts`(`SELF_HERO_PATHS`), `(content)` 컨테이너 사용처 16곳(Phase 4), `news/page.tsx`(랜딩 신규).
