...
### 🟢 nav pathname 매칭이 segment boundary 무시 (잠재 prefix collision)

- **무엇**: `isActiveGnb`·`resolveBreadcrumbSegments`는 `pathname.startsWith(item.href)`로, `resolveMobileHeader`는 `startsWith(item.href + '/')`로 매칭 — resolver마다 trailing-slash 정책이 다르고 segment boundary를 보지 않는다
- **왜**: 단일 데이터(`GNB_ITEMS`) prefix 매칭으로 시작. 현재 부모 href 5개(`/about`·`/next-gen`·`/sermons`·`/community`·`/news`)가 서로 prefix가 아니라 동작상 문제 없음
- **위험**: `/newsletter` 같은 형제 라우트가 생기면 `'/newsletter'.startsWith('/news')`가 true라 교회 소식을 잘못 활성화한다. `resolveMobileHeader`의 children·no-children 분기에 같은 `startsWith(href + '/')` catch가 중복된 점도 함수 비대화 신호다
- **마이그레이션 경로**: 옵션 B(`SITE_MAP`) 통합 시 segment-boundary 기반 셀렉터로 매칭을 단일화한다. resolver별 정책 차이와 분기 중복이 함께 사라진다
- **영향 범위**: `src/config/navigation.ts` (`isActiveGnb`·`resolveMobileHeader`·`resolveBreadcrumbSegments`)
- **발견일**: 2026-05-31 (sitemap-consistency-fix Codex 평가 검증 C2·C3·C4)

### 🟢 resolveHeroMeta subtitle comparator가 키 길이를 subtitle 길이와 비교

- **무엇**: `for (key of HERO_META) if (categoryKey.startsWith(key) && key.length > subtitle.length)` — "가장 구체적인 키 선택" 의도인데 경로 키 길이를 직전 best 키 길이가 아니라 누적 subtitle 텍스트 길이와 비교한다
- **왜**: 초기 작성 시 comparator 오류. 현재 `categoryKey`는 항상 2-세그먼트 루트(`item.href.split('/').slice(0,2)`)이고 `HERO_META` 키도 전부 2-세그먼트 루트라, 어떤 categoryKey든 `startsWith`로 정확히 1개만 매칭 → 다중 매칭 경로가 없어 도달 불가
- **위험**: 향후 중첩 `HERO_META` 키(`/sermons` + `/sermons/x`)를 추가하면, 짧은 키의 subtitle이 길 때 더 구체적인 키가 갱신되지 않아 잘못된 subtitle을 표시한다
- **마이그레이션 경로**: `key.length > bestKeyLen` 추적으로 한 줄 교정하거나 옵션 B 셀렉터로 흡수한다
- **영향 범위**: `src/components/layout/Hero/hero.config.ts` (`resolveHeroMeta` subtitle 루프)
- **발견일**: 2026-05-31 (sitemap-consistency-fix Codex 평가 검증 C5)
