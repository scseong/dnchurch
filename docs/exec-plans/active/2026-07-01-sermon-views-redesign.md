...
- [x] C3b. 설교 상세 레이아웃 정정 (사용자 지적: "이게 목업이랑 같냐"). 색만 바꾸고 구조가 어긋나 있었음 — 목업 대비 실측 후 구조 일치: 시리즈를 제목 위 텍스트 줄 → 영상 좌상단 pill("제목 00")로, 영상 우하단 재생시간 오버레이, 메타의 공유·저장 pill 제거(헤더 네이티브 공유가 OS 시트로 대체 — 카카오 포함) + `SermonMetaActions`·`useSermonShare`·`useSermonBookmark` 삭제, 메타 순서 설교자·본문·날짜, 영상 placeholder warm. 브라우저: 영상 있는 설교(썸네일+재생+duration)·시리즈 설교(pill) 목업 구조 일치 확인.
