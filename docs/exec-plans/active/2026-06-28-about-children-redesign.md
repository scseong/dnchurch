...
기존 교회 소개에 속한 페이지들을 Claude Design 디자인 시안에 맞게 리디자인한다. 모바일 기기를 고려한 UI로 정보 가독성을 높이고 불필요한 요소를 덜어내는 작업이다. 대상은 worship 예배 안내·location 오시는 길·vision 교회의 비전·welcome 환영합니다 4개 페이지다. 인사말 페이지([[2026-06-28-about-warm-redesign]])에서 만든 warm 카드 시스템·AboutTabNav를 그대로 재사용하고, 실데이터가 없는 콘텐츠는 대구동남교회용 플레이스홀더로 채운다.
...
> 진행 중 함께 나온 부수 작업(별도 커밋): warm 톤 팔레트 브라운 전환 `49caed9`, 모바일 헤더 정리 `4971400`, LayoutContainer 본문 패딩 일원화 `c6298d0`, 인사말 페이지 톤 정렬 `e6d9627`, 교회 소개 탭 내비 active 라벨 진하게 `c095d11`. 팔레트 최종값은 브라운 `#7a6654`·카드 흰색.
