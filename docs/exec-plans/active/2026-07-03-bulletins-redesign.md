...
- **D4 — 목업 세부에 더 맞춘 3가지(이미지 배지·첫 행 강조·배지 점)**
  ...
  - 후속: featured 배지의 골드 점은 이후 사용자가 뺐고, 배지 여백도 조금 좁혔다(목록 카드 다듬기).
...
- **D8 — 상세 헤더에서 날짜를 빼고 제목만 1.8rem으로 둔다(사용자 확인 반영)**
  - 문제: 상세 헤더가 날짜 라벨과 제목으로 같은 날짜를 두 번 보여줬다 — 교회 제목이 sunday_date를 그대로 담기 때문이다(dev 실데이터로 확인).
  - 해결: 상세에서 날짜를 빼고 제목만 남겼다. 제목 폰트를 2rem에서 1.8rem(`$font-size-18`)으로 낮췄다. 배경·테두리·가운데 정렬 같은 별도 장식은 넣지 않았다. 관리자 '수정' 링크는 제목과 같은 줄 오른쪽에 둔다. 목록 featured 카드는 기존 디자인(날짜·제목)을 그대로 뒀다 — 이 변경은 상세에만 넣는다.
  - 결과: 상세 헤더가 제목 한 줄로 단순해졌고, 목록은 원래 모습대로 남았다.

- **D9 — 상세 이미지에 테두리를 두르고 아이콘 크기를 width·height로 명시한다(사용자 요청)**
  - 문제: 상세 이미지에 테두리가 없었다. 공유 카드 아이콘은 `font-size`로 크기를 줬는데, 이건 `1em`을 쓰는 react-icons(링크·저장)에만 적용되고 카카오 인라인 svg(고정 px 속성)엔 적용되지 않아 세 아이콘 크기가 어긋났다. 확대 힌트 svg도 고정 px 속성이라 rem으로 커지는 주변 pill과 비율이 안 맞았다.
  - 해결:
    - `PhotoSwipe`에 `className`을 넘겨(`.image_box`에 도달) 상세 이미지 각각에 `$home-border` 1px 테두리를 컨테이너와 같은 `$radius-m`로 둘렀다.
...
| QA 다듬기(D8·D9) | 20260704-185907 | ✅ | ✅ | ✅ | 0(기존 부채만) | 실기기 QA 완료(사용자) |
