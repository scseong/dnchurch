...
- 0편·미매칭 설교자 필터의 빈 상태 개선(`isUnknownPreacher`) — 새 동작이라 이 PR 제외. 판정 기준은 "preacher 파라미터 있는데 결과 0건". 기록: `docs/tech-debt/active.md`.
- E-데이터: 연혁 4줄과 통계(사역·교구·기도일)에 실제 값을 넣는다 — 운영자 데이터 대기. prod `church_history` 설립연도 1958은 develop→main 릴리스 시점에 코드와 함께 반영한다(배포가 캐시를 비운다).
- next-gen·community·news/gallery stub 페이지 실제 구현 — 완성 시 `sitemap.ts` STATIC_PATHS에 경로 복구.

## 회고

- **잘된 것**: 가정을 실제 응답으로 검증했다. C(0편 설교자)를 "이미 해결"로 봤다가 Codex 계획 검증 지적을 받고 PostgREST REST 응답을 직접 열어 보니 박지권이 `count:0`으로 반환됨을 확인 — `sermons!inner(count)`가 0편을 안 거른다는 걸 plain SQL 재현이 아니라 실응답으로 잡았다. 브라우저 검증에서 설립연도 캐시 이슈(`force-cache`)와 title 중복 suffix도 발견했다. Codex 계획·1차 + gemini 리뷰를 반영해 서비스 필터를 표시 레이어로 옮겼다.
- **다음에 할 것**: 공유 쿼리(`allPreachers`/`allSeries`)에 필터를 걸면 그 쿼리를 쓰는 URL 해석·admin 선택까지 바뀐다 — 표시용 거르기는 표시 레이어(page)에서 한다. 0편·미매칭 같은 경계 동작은 단순 SQL 재현으로 단정하지 말고 실제 REST 응답으로 확인한다. 봇 리뷰의 일반론 널 가드 제안(`?? []`)은 데이터 출처가 이미 배열을 보장하는지 보고 판단한다(중복 가드 회피).
- **발견된 부채**: 모두 후속 작업으로 추적한다.
  - `isUnknownPreacher` 비대칭 — 시리즈엔 미매칭 가드가 있고 설교자엔 없다.
  - stub 페이지 11개가 아직 미구현이다.
  - /about 연혁·통계 데이터가 비어 있다.
