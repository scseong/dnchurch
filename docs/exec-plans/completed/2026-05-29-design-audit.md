...
- **잘된 것**: `(content)` 7도메인 36페이지를 4영역 위반 14건으로 file:line 감사했다. `context.md`(영구 SSOT)·`audit.md`(snapshot) 두 문서로 Phase 2~5 입력을 확보했다.
- **다음에 할 것**: 수치는 코드를 직접 세서 확정한다. 본 감사가 stylelint 발생 횟수를 라인으로 세고(primitive 36 vs 실제 38) `news/notices/[id]`를 코드 확인 없이 디테일로 분류해, PR #107 리뷰에서 9건을 정정했다(디테일 4→3·스켈레톤 16→17·MainContainer 5→6). 본 D4의 "디테일 4·스켈레톤 16"은 정정 전 수치이고, `audit.md`가 최신이다.
- **발견한 부채**: codex-companion이 백그라운드 호출 결과를 회수하지 못한다(미종료 프로세스 3개 잔존). 인프라 결함이라 디자인과 무관하다.
