...
- og:image 갭을 about 페이지의 `...OPEN_GRAPH_BASE` 패턴 재사용으로 닫았다. 새 추상화 없이 base 펼침 + `OG_FALLBACK_IMAGE` 상수만으로 끝냈다.
- 빌드 통과만 믿지 않고 prod 서버(빌드 산출물) 실측으로 목록 4개의 og:image가 배너 절대 URL로 나오는 것을 확인했다.
- PR 봇 지적을 코드로 검증해 처리했다. Gemini의 삼항→`||` 제안 3건은 `.gemini/styleguide.md §7`과 `getOgImageUrl` 반환 타입을 확인한 뒤 수용해 혼합 배열 타입까지 정리했고, Codex는 👍(이슈 없음)였다.
- "모든 페이지에 적용됐나" 질문에 openGraph 선언 16파일 전수 점검으로 답했다. 선언 안 한 페이지는 root layout을 상속해 원래 정상임을 확인했다.

**다음에 할 것**

- 상세 3개를 처음부터 `[{ url: x || OG_FALLBACK_IMAGE }]`로 짰으면 Gemini 지적과 추가 커밋 한 번(`5f95692`)을 아꼈다. 삼항+`[]`보다 `||`가 배열 타입을 일관되게 한다.
- exec-plan 문서를 verify-task 뒤에 또 고쳐 검증 기록을 두 번 재실행했다(`20260618-211658` → `20260619-143446` → `20260619-145827`). 문서 편집을 verify 전에 마치는 순서로.

**부채·후속**

- 페이지별 전용 공유 이미지는 후속이다. 지금은 기본 배너로 통일했다.
- 이미지 없는 상세의 fallback은 런타임 미관측이다(표본에 이미지 없는 설교·시리즈·주보가 없었다). 그런 항목이 생기면 실측한다.
- og:image tech-debt(`active.md`, PR #110 항목)는 이 작업으로 해소돼 `resolved.md`로 옮겼다.
