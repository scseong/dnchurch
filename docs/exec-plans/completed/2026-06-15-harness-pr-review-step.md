...
- **D5 — 공개 답글을 Evidence block에서 산문으로 분리한다 (기록 ≠ 공개 답글)**
  - 문제: PR #125에 답글을 `주장/대조/결과` Evidence block으로 달았더니, 라벨이 내부 스키마라 읽는 사람이 각 줄의 뜻을 못 잡았다(사용자 지적). 내 진단과 Codex 진단이 일치 — 근본 원인은 "내부 기록 포맷을 외부 청중에 그대로 적용".
  - 해결: 내부 기록(exec-plan `## PR 리뷰 대응` 표)은 감사용으로 유지하되, 공개 답글은 자연 산문으로 변환한다. 산문에 지적 확인 → 근거·타당성 → 조치 → (해당되면)대안·효과·비용을 한 흐름으로 담되 고정 라벨은 쓰지 않는다 — 라벨 나열이 바로 가독성 문제를 만들기 때문. § 8 step 4 재작성.
  - 결과: 답글이 스키마 없이 읽힌다. 내 writing 규칙 ⑤("한 흐름이면 그대로")과도 일치.
...
**잘된 것**
- PR_REVIEW 절차를 만들고, 그 PR(#125)이 스스로 리뷰받으며 절차가 단단해졌다 — 자기 절차로 자기 PR을 검증한 셈이다. 수집 레시피(`--paginate`·root id 필터)·검증 규율·답글 형식이 실제 봇 리뷰로 보강됐다.
- "중계 금지" 규율이 reviewer 오탐을 잡았다. Codex 1차 검증이 "답글 API 경로 404"를 가장 무거운 근거로 올렸으나, GitHub 문서로 직접 확인하니 우리 경로가 맞았다 — 절차의 핵심 가치를 자기 검증에서 입증.
- 봇·Codex·자체 감사 3겹이 서로의 사각을 메웠다. Codex가 봇이 놓친 "Evidence" 잔재와 체계적 근본 원인(전파 누락)을 잡았다.

**다음에 할 것**
- 공개 답글을 처음부터 산문으로 시작했으면 D5 왕복(Evidence block → 라벨 한글화 → 산문)을 줄였다. 내부 기록 형식을 외부 청중에 그대로 쓰지 말 것.
- 하네스 정책을 추가하면 전파 대상(에이전트 정의·CLAUDE 이력·hooks README·SSOT 포인터·폐기어)을 같이 점검할 것. D5 잔재가 6곳에 흩어졌고, 감사 후에도 1건이 더 남아 있었다.

**부채**
- § 8 역할이 claude-code·commit-pr-author 정의에 안 적혔다(미전파). 하네스 정책을 추가할 때 동기화 대상을 강제 점검하는 절차도 없다. 둘 다 실행에는 무해해 `docs/tech-debt/active.md`에 등록하고 § 9 확장 때 처리한다.
