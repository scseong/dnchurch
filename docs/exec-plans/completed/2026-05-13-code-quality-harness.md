...
- **2026-05-13**: PR #85 리뷰 반영. ① Gemini + Codex 모두 `docs/decisions/README.md:40`의 ADR 0008 상태 `Proposed → Accepted` 동기화 누락 지적 — `update-adr-index.mjs` 재실행으로 해소. ② Codex P2 — ADR Decision이 "14축 점검한다"고 선언하지만 skill의 `CODEX_FIRST_PASS`/`VERIFY`와 template 검증 프롬프트에 14축 체크리스트 실제 인용 없음 지적. 대응 — 본 PR 범위 외임을 ADR 메커니즘 2 끝 + 도입 절차 시작에 강조 박스로 명시. 14축 실제 삽입은 후속 `agent-quality-guidance` PR에서.
