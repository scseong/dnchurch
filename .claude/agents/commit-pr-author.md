...
- **PR 초안 생성** — base develop과의 diff + 포함 commit 메시지들 → `[Type] Title` + 본문(template 매핑) + label 후보 + `--assignee "@me"` + base `develop`
...
- **commit 후 PR 직전 호출인데 base develop과 diff 0** — "develop과 차이 없음. PR 생성 불필요." 후 종료
