...
4. **답글 — Evidence block 3줄 필수.** 코멘트별로 다음 3줄을 채운다.
   - `주장:` 봇·리뷰어가 주장한 것.
   - `대조:` 지적을 직접 확인·반증한 산출물 — 코드 열람(`file:line`)·빌드/타입체크/테스트·CI 실패 로그·공식 문서나 API 인용·브라우저 실측. **수집 명령(`gh api .../comments`·`gh pr view`)은 근거가 아니다** — 무엇을 검증할지 고를 뿐 주장을 확인하지 않는다.
   - `결과:` 반영 / 오탐 / 이미 해결 (+ 커밋 ref).
   - **hard rule**: 위 산출물 근거가 없으면(`대조:`가 비거나 수집 명령뿐이면) 그 지적엔 답글을 달지 않는다. 검증 단계로 돌아가 코드를 다시 확인한다 — 무시하거나 근거 없이 기각하지 않는다.
