...
- **스크린샷 표는 label과 이미지를 같은 행에 두지 않는다 — label을 위 행에, 이미지를 바로 아래 행에 배치한다.** 이미지는 세로가 길어 같은 행에 label을 두면 텍스트가 이미지에 묻혀 안 보인다. 한 화면 = 2행: 위 행에 화면 label(나머지 열은 비움), 바로 아래 행에 데스크톱·모바일 이미지. 첨부할 화면은 비워두지 말고 구체적으로 나열하고, 이미지 셀에는 "여기에 드래그&드롭" 안내. 개인 데이터 화면은 작성자가 캡처하지 않고 사용자가 붙인다.

  스크린샷 표 형태:
  ```markdown
  | 데스크톱 | 모바일 |
  | --- | --- |
  | **(a) 마이 페이지 전체 — 프로필 + 연속 카드** | |
  | _여기에 드래그&드롭_ | _여기에 드래그&드롭_ |
  | **(b) 프로필 편집** | |
  | _여기에 드래그&드롭_ | _여기에 드래그&드롭_ |
  ```

  ❌ 나쁨: 보안 설명 4문장을 한 문단에, 문제 해결 9건을 각 항목 한 문단(`문제 …, 해결 …`)에 뭉침, 스크린샷 표에서 label을 이미지와 같은 행에 둠
  ✅ 좋음: 계층별 소제목 + 주장 bullet, 항목별 `- 문제:`/`- 해결:` 두 줄, 스크린샷은 label 행 위·이미지 행 아래 (예시 PR #152)
