...
2. **한 문장에 한 가지, 문장이 여러 개면 나눠 보이게** — 양보·이유·예외를 한 문장에 겹치지 않는다. 두 가지면 bullet으로 쪼갠다. 한 단락이나 표 셀에 문장이 2개 이상 연속되면 bullet이나 줄바꿈으로 나눠 한눈에 들어오게 한다 (마크다운 표 셀은 실제 개행이 안 되므로 `<br>`을 쓴다).
...
| ⑦ 한 단락·표 셀에 문장 2개+ 연속 | bullet 또는 줄바꿈으로 분리 — 한눈에 안 들어오면 쪼갬. 표 셀은 `<br>`로 줄바꿈(마크다운 셀은 실제 개행 불가) |
...
```
이 PR은 리뷰 대응 절차를 추가한다. 봇 지적을 코드 확인 없이 옮기면 틀린다. #118·#119에서 오탐이 두 건 나왔다. 그래서 답글마다 Evidence block을 강제한다.
```
...
✅ 좋음 (표 셀은 `<br>`로 줄바꿈):
```
| 선택 이유 | 리뷰가 비동기로 와 결정적 타이밍이 없음<br>→ 조건부 진입으로 처리 |
```
