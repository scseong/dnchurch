...
<!-- UI가 바뀌는 작업이면 필수. UI 변경이 없으면 이 섹션을 통째로 삭제하세요.
     이미지는 이 입력창에 드래그&드롭으로 붙입니다(GitHub이 링크로 바꿔 줌).
     데스크톱·모바일을 함께 담고, 기존 화면이 바뀌면 변경 전/후를 나란히 두세요. -->
...
- **운영 리스크**: <!-- 환경변수·마이그레이션·배포 순서. 없으면 "없음" -->
- **QA 후속**: <!-- 시드 데이터·실기기 확인 등 머지 후 할 일. 없으면 "없음" -->
- **롤백 시 영향**: <!-- 없으면 "없음" -->
